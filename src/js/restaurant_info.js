  ...
  fetchRestaurantFromURL((error, restaurant) => {
    if (error) { // Got an error!
      ...
      L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.jpg70?access_token={mapboxToken}', {
        mapboxToken: 'pk.eyJ1IjoidW1icmVvbmh1Z3MiLCJhIjoiY2pta2F0dmp0MDBwbjNwbHF3ZGhxNnhrOCJ9.LmCaFd_cBQU7tygoMtTxAA',
        maxZoom: 18,
...
createReviewHTML = (review) => {
  ...
  name.innerHTML = review.name + " says...";
  li.appendChild(name);

  const rating = document.createElement('div');
  rating.className = "star-rating-box";
  rating.setAttribute('aria-label', 'Rating: ' + review.rating + " stars");
  // rating.innerHTML = `Rating: ${review.rating}`;
  let starRating = ``;
  for (let i=0;i < review.rating; i++){
      starRating += `<i class="fa fa-star star-rating"></i>`;
  }
